...
class HuatuoChatbot:
    ...
    def init_components(self):
        ...
        if "huatuogpt-vision-7b" in d.lower():
            ...
            missing_keys = loading_info[
                "missing_keys"
            ]  # keys exists in model architecture but does not exist in ckpt
            unexpected_keys = loading_info[
                "unexpected_keys"
            ]  # keys exists in ckpt but are not loaded by the model
    ...
    def preprocess_huatuo(
        self, convs: list, return_tensors
    ) -> list:  # tokenize and concat the coversations
        input_ids = None
        ...
        for ind in range(round_num):
            ...
            cur_input_ids = self.tokenizer_image_token(
                prompt=h, return_tensors=return_tensors
            )
            ...
            cur_input_ids = self.tokenizer(
                g, add_special_tokens=False, truncation=True, return_tensors="pt"
            ).input_ids[0]
            input_ids = torch.cat([input_ids, cur_input_ids])
    ...
    def get_image_tensors(self, images):
        ...
        if len(list_image_tensors) == 0:
            list_image_tensors.append(
                torch.zeros(3, crop_size["height"], crop_size["width"]).to(self.device)
            )
    ...
    def inference(self, text, images=None):
        ...
        text = self.insert_image_placeholder(
            text, len(images) if None not in images else 0
        )

        conv = self.get_conv_without_history(text)
        input_ids = (
            self.preprocess(conv, return_tensors="pt").unsqueeze(0).to(self.device)
        )

        if len(images) > 0:
            list_image_tensors = self.get_image_tensors(images)
            image_tensors = (
                torch.stack(list_image_tensors).to(dtype=torch.bfloat16).to(self.device)
            )
        else:
            image_tensors = None
        ...
        with torch.inference_mode():
            output_ids = self.model.generate(
                input_ids, images=image_tensors, use_cache=True, **self.gen_kwargs
            )
        answers = []
        for output_id in output_ids:
            answers.append(
                self.tokenizer.decode(output_id, skip_special_tokens=True).strip()
            )
        return answers
    ...
    def chat(
        ...
        generation_kwargs = dict(
            inputs=input_ids,
            images=(
                image_tensors.to(dtype=torch.bfloat16)
        ...
        with torch.inference_mode():
            ...
            for new_text in streamer:
                if sep in new_text:
                    new_text = self.remove_overlap(
                        generated_text, new_text[: -len(sep)]
                    )
